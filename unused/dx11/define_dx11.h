...
namespace PO::Dx11
{
	...
	using ShaderVS = ID3D11VertexShader;
	using ShaderHS = ID3D11HullShader;
	using ShaderDS = ID3D11DomainShader;
	using ShaderGS = ID3D11GeometryShader;
	using ShaderPS = ID3D11PixelShader;
	using ShaderCS = ID3D11ComputeShader;
	...
	inline HRESULT create_shader_reflection(ShaderReflection** output, const std::byte* buffer, size_t length) { return D3DReflect(buffer, static_cast<UINT>(length), __uuidof(ID3D11ShaderReflection), reinterpret_cast<void**>(output)); }
	...
	//HRESULT create_output_texture2D(Texture2D** output, Device& dev, DXGI::Format format, uint32_t x, uint32_t y) noexcept;
	...
	HRESULT create_unordered_access_texture2D(Texture2D** output, Device& dev, DXGI::Format format, uint32_t x, uint32_t y, uint32_t mipmap = 1, const std::byte** buffer = nullptr, const uint32_t* line_count = 0) noexcept;

	HRESULT create_texture2D_uav(UnorderedAccessView** output, Device& dev, Resource&, DXGI::Format, uint32_t mipmap = 0) noexcept;
	HRESULT create_texture2D_rtv(RenderTargetView**, Device&, Resource&, DXGI::Format, UINT mipmap = 0) noexcept;
	HRESULT create_texture2D_srv(ShaderResourceView**, Device&, Resource&, DXGI::Format, UINT mipmap_start, UINT mipmap_count) noexcept;

	HRESULT create_vertex(Buffer**, Device&, const std::byte* buffer, UINT width) noexcept;
	HRESULT create_index(Buffer**, Device&, const std::byte* buffer, UINT width) noexcept;

	inline HRESULT create_shader(ShaderVS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreateVertexShader(buffer, static_cast<UINT>(length), nullptr, output); }
	inline HRESULT create_shader(ShaderHS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreateHullShader(buffer, static_cast<UINT>(length), nullptr, output); }
	inline HRESULT create_shader(ShaderDS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreateDomainShader(buffer, static_cast<UINT>(length), nullptr, output); }
	inline HRESULT create_shader(ShaderGS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreateGeometryShader(buffer, static_cast<UINT>(length), nullptr, output); }
	inline HRESULT create_shader(ShaderPS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreatePixelShader(buffer, static_cast<UINT>(length), nullptr, output); }
	inline HRESULT create_shader(ShaderCS** output, Device& dev, const std::byte* buffer, size_t length) noexcept { return dev.CreateComputeShader(buffer, static_cast<UINT>(length), nullptr, output); }
	HRESULT create_gs_shader_with_stream_out(ShaderGS** output, Device& dev, const std::byte* buffer, size_t length, ShaderReflection&, bool need_stream_out, UINT stream_out);

	//HRESULT create_rtv(RenderTargetView** output, Device& dev, SwapCahin& sw, DXGI::Format format) noexcept;
	//HRESULT create_rtv(RenderTargetView**, Device&, Resource&, DXGI::Format, uint32_t mipmap) noexcept;
	//HRESULT create_srv(ShaderResourceView**, Device&, Resource&, DXGI::Format, uint32_t mipmap, uint32_t mipmap_count, uint32_t array, uint32_t array_count) noexcept;

	struct shader_stage_clear_flag
	{
		bool vs = false;
		bool hs = false;
		bool ds = false;
	...
	void clear_stage(const shader_stage_clear_flag&, DeviceContext&);
...
namespace PO
{
	namespace Dx11
	{
		/*
		//using namespace Dx;
		using command_list = CComPtr<ID3D11CommandList>;

		using device = CComPtr<ID3D11Device>;
		using context = CComPtr<ID3D11DeviceContext>;
		using resource = CComPtr<ID3D11Resource>;
		using buffer = CComPtr<ID3D11Buffer>;
		using shader_hs = CComPtr<ID3D11HullShader>;
		using shader_ds = CComPtr<ID3D11DomainShader>;
		using shader_vs = CComPtr<ID3D11VertexShader>;
		using shader_ps = CComPtr<ID3D11PixelShader>;
		using shader_cs = CComPtr<ID3D11ComputeShader>;
		using shader_gs = CComPtr<ID3D11GeometryShader>;

		using input_layout_desc = D3D11_INPUT_ELEMENT_DESC;
		using input_layout = CComPtr<ID3D11InputLayout>;
		using tex2_des = D3D11_TEXTURE2D_DESC;
		using tex2 = CComPtr<ID3D11Texture2D>;
		using tex1_des = D3D11_TEXTURE1D_DESC;
		using tex1 = CComPtr<ID3D11Texture1D>;
		using tex3_des = D3D11_TEXTURE3D_DESC;
		using tex3 = CComPtr<ID3D11Texture3D>;

		using view = CComPtr<ID3D11View>;
		using view_unorder_access = CComPtr<ID3D11UnorderedAccessView>;
		using view_render_target_des = D3D11_RENDER_TARGET_VIEW_DESC;
		using view_render_target = CComPtr<ID3D11RenderTargetView>;
		using view_depth_stencil = CComPtr<ID3D11DepthStencilView>;
		using view_shader_resource_des = D3D11_SHADER_RESOURCE_VIEW_DESC;
		using view_shader_resource = CComPtr<ID3D11ShaderResourceView>;

		using state_sampler = CComPtr<ID3D11SamplerState>;

		using state_raterizer = CComPtr<ID3D11RasterizerState>;
		using state_blend = CComPtr<ID3D11BlendState>;
		using state_depth_stencil = CComPtr<ID3D11DepthStencilState>;

		struct output_merga
		{
			std::array<ID3D11RenderTargetView*, D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT> storage;
			view_depth_stencil depth_storage;
			void insert(const view_render_target& vt, uint16_t count);
			void insert(view_depth_stencil ds);
			void clear();
			output_merga();
			output_merga(output_merga&&);
			output_merga(const output_merga&);
			~output_merga();
		};

		void set_output_merga(context& c, const output_merga& om);
		void clear_compute_state(context& c);
		void clear_renderer_state(context& c);
		*/

		//view_render_target cast_render_target(device& c, tex2& t, uint32_t mipmap = 0);
