...
struct Simulate3DFloatWith2DUnorm4 : public compute_resource
{
	...
	struct property
	{
		...
		AdressType type = AdressType::Clamp;
		struct renderer_data_append
		{
			buffer_constant bc;
		};
		...
		{
			shader_storage<uint32_t4, float4, uint> ss{ simulate_size , Factor,  type == AdressType::Wrap ? 0 : (type == AdressType::Wrap ? 1 : 2 ) };
...
struct GeneratorMetaBall3D : public compute_resource
{
	struct property
	{
		unordered_access_view<tex3> shape_output;
		shader_resource_view<tex2> area;
		shader_resource_view<buffer_structured> original_point;
		uint32_t3 texture_size;
	};
	GeneratorMetaBall3D(creator& c);
	const element_requirement& requirement();
};

struct PointFilter : public compute_resource
{
	struct property
	{
		unordered_access_view<buffer_structured> input;
		unordered_access_view<buffer_structured> output;
		size_t input_size;
		...
		void update(creator& c, renderer_data_append& reda)
		{
			reda.ss.create(c, ss_des);
		}
	};
	PointFilter(creator& c);
	const element_requirement& requirement();
};

struct fbmgenerator : public compute_resource
{ 
	struct property
	{
		shader_resource_view<buffer_structured> random_point;
		shader_resource_view<tex3> input;
		sample_state::description ss_des = sample_state::default_description;
		unordered_access_view<tex3> output;
		uint3 input_size = uint3{0, 0, 0};
		float scale = 1.0;
		float value_scale = 0.5;
		float ValueMulity = 0.1;
		struct renderer_data_append
		{
			buffer_constant bc;
			sample_state ss;
		...
		void update(creator& c, renderer_data_append& rda)
		{
			shader_storage<float, float, float> ss{ scale, value_scale, ValueMulity };
			rda.bc.create_pod(c, ss);
			rda.ss.create(c, ss_des);
		}
	};
	fbmgenerator(creator& c);
	const element_requirement& requirement();
};

struct fbmWorleyNoiseGenerator : public compute_resource
{
	struct property
	{
		unordered_access_view<tex3> fbmOutput;
		float3 Block = float3{ 10.0, 10.0, 10.0 };
		...
		void update(creator& c, renderer_data_append& rda)
		{
			shader_storage<float3, float3, float, uint, float, float, float, uint3> ss{ Block, Shift, Mulity, Octaves,  Frequence , Lacunarity , Gain};
			rda.bc.create_pod(c, ss);
		}
	};
	fbmWorleyNoiseGenerator(creator& c);
	const element_requirement& requirement();
};