...
float ValuePerlinNoiseRand(float2 UV, float2 Block)
{
    ...
    float2 Rate = frac(TruePoi);
    return 
    lerp(
        lerp(
            rand(Vertex), rand(Vertex + float2(1, 0)), PerlinRate(Rate.x)
        ),
        lerp(
            rand(Vertex + float2(0, 1)), rand(Vertex + float2(1, 1)), PerlinRate(Rate.x)
        ),
        PerlinRate(Rate.y)
    );
}

float fbm_ValuePerlinNoiseRand(in uint octaves, in float frequency, in float lacunarity, in float gain, float2 UV, float2 Block)
{
    float amplitude = gain;
    float total = 0.0;
    for (uint i = 0; i < octaves; ++i)
    {
        total += ValuePerlinNoiseRand(UV * frequency, Block) * amplitude;
...
float WorleyNoiseRand(float3 n, float multy)
{
    ...
    for (uint count = 0; count < 27; ++count)
    {
        ...
        float3 p = floor(n) + dir;
        float3 rate = float3(rand(p.xyz), rand(p.zxy), rand(p.yzx));
        float3 pre_rate = rate + dir - frac(n);
        float d = length(pre_rate) * multy;
        dis = min(dis, d);
    }
    return 1.0 - dis;
...
float fbm_WorleyNoiseRand(in uint octaves, in float frequency, in float lacunarity, in float gain, in float3 Point, float multy)
{
    float amplitude = gain;
    float total = 0.0;
    for (uint i = 0; i < octaves; ++i)
    {
        total += WorleyNoiseRand(Point * frequency, multy) * amplitude;
        frequency *= lacunarity;
        amplitude *= gain;
    }
    return total;
