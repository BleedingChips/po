...
namespace PO::Dx11
{
	struct ComWrapper
	{
		...
		template<typename T> static T** ref(Tool::intrusive_ptr<T, ComWrapper>& ptr) { assert(!ptr); return &ptr.m_ptr; }
	};

	template<typename T> using ComPtr = Tool::intrusive_ptr<T, ComWrapper>;
	...
	struct Context;
	using ContextPtr = Tool::intrusive_ptr<Context>;
	...
	private:
		...
		Tool::intrusive_ptr<Command> m_waitiing;
		Tool::intrusive_ptr<Command> m_buffer;
	...
	private:
		...
		std::deque<Tool::intrusive_ptr<CommandInterface>> m_list;
