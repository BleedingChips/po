...
namespace PO
{
	namespace Dx11
	{
		...
		sub_viewport_projection_3d::sub_viewport_projection_3d(float2 view_size, float2 view_left_top, float angle, float2 far_near_plane, float2 avalible_depth)
		...
		renderer_default::renderer_default(value_table& vt) :
			stage_context(vt.get<stage_context>()),
		...
		renderer_defer_default_3d::renderer_defer_default_3d(value_table& vt) :
			creator(vt.get<creator>()), renderer(vt), viewer(renderer.back_buffer)
		{
