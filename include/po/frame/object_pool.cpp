...
namespace PO::ECSFramework
{
	namespace Implement
	{
		...
		{
			size_t min_total_size = sizeof(resource_page) + (sizeof(element_scription) + real_size + real_alignas) * min_count;
			size_t total_size = (min_total_size / 1024 + (min_total_size % 1024 == 0 ? 0 : 1)) * 1024;
			size_t element_count = (total_size - sizeof(resource_page)) / (sizeof(element_scription) + real_size + real_alignas);
		...
		resource_page_manager::resource_page_manager(size_t type, size_t aligned) :
			type_size(type), type_alignas(aligned),
			real_size(type < sizeof(nullptr) ? sizeof(nullptr) : type), real_alignas(aligned < sizeof(nullptr) ? sizeof(nullptr) : aligned), avalible_top(nullptr), page_top(nullptr)
		{
		...
		resource_page_manager::resource_page_manager(resource_page_manager&& spm) : type_size(spm.type_size), type_alignas(spm.type_alignas), real_size(spm.real_size), real_alignas(spm.real_alignas)
		{
	...
	void* object_pool::allocate(std::type_index key, size_t type_size, size_t alignas_size)
	{
		...
		Implement::resource_page_manager& ref = all_resource.lock([&](lock_type& lt) -> Implement::resource_page_manager& {
			...
			if (ite != lt.end())
			{
				assert(ite->second.is_same_size(type_size, alignas_size));
				return ite->second;
			}
			else
			{
				auto ite = lt.try_emplace(key, type_size, alignas_size);
				assert(ite.second);
	...
	void object_pool::release(void* resource_ptr) noexcept
	{
