...
namespace
{
	...
	std::tuple<DWORD, DWORD> translate_style(PO::Win32::Style style)
	{
		switch (style)
		{
		case PO::Win32::Style::Normal:
	...
	LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case UD_REQUEST_QUIT:
		{
			...
			PO::Win32::Implement::Control* ptr = reinterpret_cast<PO::Win32::Implement::Control*>(data);
			delete ptr;
	...
	struct GobalManager
	{
		PO::Tool::intrusive_ptr<PO::Win32::Implement::Control> create_control(const PO::Win32::FormProperty& pro);
		~GobalManager();
	private:
		...
		size_t count = 0;
		std::vector<std::tuple<std::promise<void>&, const PO::Win32::FormProperty&, PO::Win32::Implement::Control*>> requests;
				...
				if (!ins->requests.empty())
				{
					for (auto& ite : ins->requests)
					{
						...
						if (handle != nullptr)
						{
							...
							std::get<0>(ite).set_exception(std::make_exception_ptr(PO::Win32::Error::CreateWindowFauit{ GetLastError() }));
					}
					ins->requests.clear();
				}
				else if (ins->count == 0)
	...
	PO::Tool::intrusive_ptr<PO::Win32::Implement::Control> GobalManager::create_control(const PO::Win32::FormProperty& pro)
	{
		std::promise<void> promise;
		auto future = promise.get_future();
		PO::Tool::intrusive_ptr<PO::Win32::Implement::Control> control = new PO::Win32::Implement::Control{};
		{
