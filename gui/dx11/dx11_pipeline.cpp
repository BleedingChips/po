...
namespace PO
{
	namespace Dx11
	{
		namespace Implement
		{
			...
			void input_assember_context_t::bind(input_assember_d& id)
			{
			...
			void vertex_shader_context_t::bind(const vertex_shader_d& vs)
			{
			...
			/*****  raterizer_context_t   ******************************************************************************************/
			void raterizer_context_t::bind(const raterizer_d& rs)
			{
			...
			void raterizer_context_t::unbind()
			{
			...
			void pixel_shader_context_t::bind(const pixel_shader_d& vs)
			{
			...
			void output_merge_context_t::bind(const output_merge_d& od)
			{
				unbind();
			...
			void output_merge_context_t::clear_render_target(output_merge_d& omd, size_t solt, float4 color)
			{
				if (omd.render_array.size() > solt)
					cp->ClearRenderTargetView(omd.render_array[solt], &color.x);
			}

			void output_merge_context_t::clear_render_target(output_merge_d& omd, float4 color)
			{
			...
			void output_merge_context_t::clear_depth(output_merge_d& omd, float depth)
			{
				if (omd.depth != nullptr)
					cp->ClearDepthStencilView(omd.depth, D3D11_CLEAR_DEPTH, depth, 0);
			}

			void output_merge_context_t::clear_stencil(output_merge_d& omd, uint8_t ref)
			{
				if (omd.depth != nullptr)
					cp->ClearDepthStencilView(omd.depth, D3D11_CLEAR_STENCIL, 0.0f, ref);
			}

			void output_merge_context_t::clear_depth_stencil(output_merge_d& omd, float depth, uint8_t ref)
			{
				if (omd.depth != nullptr)
			...
			void draw_range_context_t::draw(const draw_range_d& d)
			{
				auto& var = d.data;
				if (var.able_cast<draw_range_d::vertex_d>())
					return cp->Draw(var.cast<draw_range_d::vertex_d>().vertex_count, var.cast<draw_range_d::vertex_d>().start_vertex_location);
				if (var.able_cast<draw_range_d::index_d>())
				{
					auto& ind = var.cast<draw_range_d::index_d>();
					return cp->DrawIndexed(ind.index_count, ind.start_index_location, ind.base_vertex_location);
				}
				if (var.able_cast<draw_range_d::instance_d>())
				{
					auto& ins = var.cast<draw_range_d::instance_d>();
					return cp->DrawInstanced(ins.vertex_pre_instance, ins.instance_count, ins.start_vertex_location, ins.start_instance_location);
				}
				if (var.able_cast<draw_range_d::instance_index_d>())
				{
					auto& in = var.cast<draw_range_d::instance_index_d>();
					return cp->DrawIndexedInstanced(in.index_pre_instance, in.instance_count, in.start_index_location, in.base_vertex_location, in.start_instance_location);
				}
				if (var.able_cast<draw_range_d::dispatch_d>())
				{
					auto& in = var.cast<draw_range_d::dispatch_d>();
					return cp->Dispatch(in.X, in.Y, in.Z);
				}
			}

			/*****  compute_shader_context_t   ******************************************************************************************/
			void compute_shader_context_t::bind(const compute_d& cd)
			{
			...
			void compute_shader_context_t::unbind()
			{
