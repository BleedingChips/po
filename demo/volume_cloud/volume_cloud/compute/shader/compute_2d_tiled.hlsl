...
float2 TilessWorleyPointDir(float2 uv, uint start, uint total_count)
{
...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    OutputTexture[DTid.xy] = //(TilessWorleyPointDir(Poi, 0, 200) + 1.0) / 2;
    TilessWorleyPoint2(Poi.xy, 10, 0, 50) * pow(0.5, 1) 
    + TilessWorleyPoint2(frac(Poi.xy * 2.0), 10, 0, 50) * pow(0.5, 2)
