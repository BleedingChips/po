...
inline bool SaveToDDS(defer_renderer_default& dfd, const tex2& t, const std::u16string& name, DXGI_FORMAT DF = DXGI_FORMAT_UNKNOWN)
{
	DirectX::ScratchImage SI;
	if (!SUCCEEDED(DirectX::CaptureTexture(dfd.dev, dfd.get_context().imp->ptr, t.ptr, SI))) return false;
...
inline bool SaveToDDS(defer_renderer_default& dfd, const tex3& t, const std::u16string& name, DXGI_FORMAT DF = DXGI_FORMAT_UNKNOWN)
{
	DirectX::ScratchImage SI;
	if (!SUCCEEDED(DirectX::CaptureTexture(dfd.dev, dfd.get_context().imp->ptr, t.ptr, SI))) return false;
	if (DF != DXGI_FORMAT_UNKNOWN)
...
inline bool SaveToTGA(defer_renderer_default& dfd, const tex2& t, const std::u16string& name, DXGI_FORMAT DF = DXGI_FORMAT_UNKNOWN)
{
	DirectX::ScratchImage SI;
	if (!SUCCEEDED(DirectX::CaptureTexture(dfd.dev, dfd.get_context().imp->ptr, t.ptr, SI))) return false;
	if (DF != DXGI_FORMAT_UNKNOWN)
